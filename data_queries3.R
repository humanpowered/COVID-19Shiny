...
county <- county %>%
  ...
  mutate(total_cases = cases - lag(cases, default = cases[1]))
county <- county %>%
  group_by(state, county) %>%
  mutate(total_deaths = deaths - lag(deaths, default = deaths[1]))

county <- county %>% rename(new_confirmed_cases = cases)
county <- county %>% rename(new_deaths = deaths)
